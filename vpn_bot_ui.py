...
def _limited_users_overview_payload(
    users: dict, page: int = 0, page_size: int = PAGE_SIZE
):
    if not users:
        return "Список лимитных пользователей пуст", None
    sorted_users = sorted(
        users.items(), key=lambda x: (not str(x[0]).isdigit(), str(x[0]))
    )
    ...
    for tg_id, rec in chunk:
        ...
        keyboard.append(
            [InlineKeyboardButton(btn_label, callback_data=f"lu:open:{tg_id}")]
        )
    if total_pages > 1:
        nav_row = []
        if page > 0:
            nav_row.append(
                InlineKeyboardButton("<<", callback_data=f"lu:list:{page - 1}")
            )
        nav_row.append(
            InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="lu:list")
        )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Изменить лимит", callback_data=f"lu:limit:{target_uid}"
            )
        ],
        [InlineKeyboardButton("Порт ключей", callback_data=f"lu:port:{target_uid}")],
        [
            InlineKeyboardButton(
                "Переименовать профиль", callback_data=f"lu:sel:{target_uid}:ren"
            )
        ],
        [
            InlineKeyboardButton(
                "Удалить профиль", callback_data=f"lu:sel:{target_uid}:del"
            )
        ],
    ]
    if viewer_is_owner:
        keyboard.append(
            [
                InlineKeyboardButton(
                    "Удалить лимитного пользователя",
                    callback_data=f"lu:rmask:{target_uid}",
                )
            ]
        )
