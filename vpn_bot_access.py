...
def init_access_db():
    ...
    if 'max_active' not in cols:
        conn.execute('ALTER TABLE limited_users ADD COLUMN max_active INTEGER NOT NULL DEFAULT 3')
        conn.execute(f'UPDATE limited_users SET max_active = COALESCE(max_creates, {DEFAULT_LIMITED_MAX_ACTIVE})')
...
def save_admins(admins: set):
    ...
    conn.executemany('INSERT OR IGNORE INTO admins (user_id) VALUES (?)', [(x,) for x in admin_ids])
    if admin_ids:
...
def get_access_attempt_identity(user_id: int | str):
    ...
    conn = access_conn()
    cur = conn.cursor()
    cur.execute('SELECT full_name, username FROM access_attempts WHERE user_id=?', (uid,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return '', ''
